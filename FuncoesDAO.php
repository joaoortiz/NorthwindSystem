...
function listarClientes($vConn, $letra){
    
    $sqlLista = "Select CustomerID, CompanyName, ContactName, City, Country, Phone, Status from customers where CustomerID like '$letra%'";
    $rsLista = mysqli_query($vConn, $sqlLista) or die(mysqli_error($vConn));
    
    return $rsLista; //retornando rsult com os dados do banco
}

function listarFuncionarios($vConn){
    $sqlLista = "Select EmployeeID, FirstName, LastName, City, Country, HomePhone from employees";
    $rsLista = mysqli_query($vConn, $sqlLista) or die(mysqli_error($vConn));
    
    return $rsLista; //retornando rsult com os dados do banco
}

function listarCategorias($vConn){
    $sqlLista = "Select * from categories";
    $rsLista = mysqli_query($vConn, $sqlLista) or die(mysqli_error($vConn));
    
    return $rsLista; //retornando rsult com os dados do banco
}

function listarProdutos($vConn, $cat){
    if($cat == 0)
        $sqlLista = "Select P.ProductID, P.ProductName, C.CategoryName, S.CompanyName, P.QuantityPerUnit, P.UnitsInStock, P.UnitPrice from Products P, Suppliers S, Categories C where C.categoryID = P.categoryID and P.supplierID = S.SupplierID";
    else
        $sqlLista = "Select P.ProductID, P.ProductName, C.CategoryName, S.CompanyName, P.QuantityPerUnit, P.UnitsInStock, P.UnitPrice from Products P, Suppliers S, Categories C where P.categoryID = $cat and C.categoryID = P.categoryID and P.supplierID = S.SupplierID";        
    $rsLista = mysqli_query($vConn, $sqlLista) or die(mysqli_error($vConn));
...
function listarOrdens($vConn, $dataIn, $dataFin, $tipo){
    
    if($tipo == 0)
        $sqlVendas = "Select * from orders O, employees E, Shippers S where O.EmployeeID = E.EmployeeID and O.ShipVia = S.ShipperID";    
    else if($tipo == 1)
        $sqlVendas = "Select * from orders O, employees E, Shippers S where O.EmployeeID = E.EmployeeID and O.ShipVia = S.ShipperID and O.orderDate between '$dataIn' and '$dataFin'";    
...
function listarVendasFuncionario($vConn, $id){
    $sqlVendas = "Select * from orders O, employees E, Shippers S where O.EmployeeID like '$id' and O.EmployeeID = E.EmployeeID and O.ShipVia = S.ShipperID order by O.OrderDate";    
    $rsVendas = mysqli_query($vConn, $sqlVendas) or die(mysqli_error($vConn));
...
function contarVendas($vConn,$idCli){
    $sqlVendas = "Select * from orders where customerID like '$idCli'";
    $rsVendas = mysqli_query($vConn, $sqlVendas) or die(mysqli_error($vConn));
    
    return mysqli_num_rows($rsVendas);
}
